...
void fastestDescent(vector<double>& result, double(&operatorA)[(M + 1) * (N + 1)][(M + 1) * (N + 1)], const vector<double>& F, const double delta, const double h1, const double h2, const GridSize& gridSize) {
    ...
    do {
        ...
        //if (world_rank == 0) {
        //    print_vector_double(wK, gridSize);
        //}
...
int main(int argc, char** argv) {
    ...
    if (world_rank == 0) {
        ...
        //print_vector_double(F, gridSize);
    }
    ...
    MPI_Bcast(operatorA, (gridSize.m + 1) * (gridSize.n + 1) * (gridSize.m + 1) * (gridSize.n + 1), MPI_DOUBLE, 0, MPI_COMM_WORLD);

    if (world_rank == 0) {
        cout << "Operator A successfully MPI_Bcast" << endl << endl;
    }
    ...
    if (world_rank == 0) {
        ...
        ofstream output("mpi_result160180p32.txt");
        for (int i = 0; i < gridSize.m + 1; i++) {
            for (int j = 0; j < gridSize.n + 1; j++) {
                output << result[i * (gridSize.n + 1) + j] << " ";
            }
            output << endl;
        }
    }
