...
void doOperator(const vector<vector<double>>& mathOperator, const vector<double>& w, vector<double>& result, const GridSize gridSize) {
    for (int i = gridSize.n + 2; i < mathOperator.size() - gridSize.n - 2; i++) {
        double tmp = 0.0;
        tmp += mathOperator[i][i] * w[i];
        tmp += mathOperator[i][i - 1] * w[i - 1];
        tmp += mathOperator[i][i + 1] * w[i + 1];
        tmp += mathOperator[i][i - gridSize.n - 1] * w[i - gridSize.n - 1];
        tmp += mathOperator[i][i + gridSize.n + 1] * w[i + gridSize.n + 1];
        result[i] = tmp;
    }
    return;
